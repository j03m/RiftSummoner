...
function saveCreds(id, pass, cb){
    redisWrap.set(credentialsNameSpace+id, pass, function(err, result){
		if (err){
			errors(500, "failed to save creds", err, cb);
		}else{
			cb(err, result);
		}
    });
...
function makeAuthToken(userToken, callback){
    ...
    redisWrap.multi(
        [["set", sessionNameSpace+sessionToken, userToken],
            ...
            if (err){
			   errors(500, "Failed to set session token in Redis: " + JSON.stringify(err), err, callback);
            }else if (replies[0]!="Ok" && replies[1]!=1){
				errors(500, "Failed to set session token in Redis: " + JSON.stringify(replies), undefined, callback);
            }else{
...
function verify(id, data, hash, cb){
    ...
    redisWrap.get(credentialsNameSpace+id, function(err, secret){
        if (err){
			errors(500, "failed to find credentials for:"+id + " in redis", err, cb);
        }else{
            ...
            if (serverHash == hash){
                ...
                errors(403, "Id: " + id + " Data:" + data + " Hash:" + hash + " did not validate.", err, cb);
            }
        }
    });
}
