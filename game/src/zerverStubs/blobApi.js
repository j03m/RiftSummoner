    ...
    var apiRefresh    = false,
        ...
        apiRoot       = "blobApi",
        apiObj        = {},
        apiFunctions  = {"saveBlob":true,"createNewPlayer":true,"getNewAuthTokenAndBlob":true,"getBlob":true,"getAuthToken":true},
        apiData       = null,
        apis          = {},
    ...
    function setupSingleAPI () {
        jc[apiName] = setupFunctions(apiObj, apiFunctions, [ apiRoot ]);
    }
                    ...
                    if (errorHandlers.length) {
                        errorHandlers.forEach(function (handler) {
                            try {
                                ...
                                jc.log(['zerverpipe'], err);
                            }
                        });
                    }
                    else{
                        jc.log(['zerverpipe'], error);
                    }
                    return;
                }
    ...
    function ajaxPost (url, data, callback) {
        ...
        var timeout = jc['ZERVER_TIMEOUT'] || XHR_TIMEOUT;
    ...
    function openWSStream (onMessage, onClose) {
        ...
        try {
            var wsCtor = jc['MozWebSocket'] || jc.WebSocket;
            conn = new wsCtor('ws://'+jc.location.host+apiSocketPath+'stream?id='+apiSocketID+'&_='+(+new Date()), 'zerver-debug');
        }
        catch (err) {
            setTimeout(function () {
                finish(false);
    ...
    function openStream (onMessage, onClose) {
        if ((jc['MozWebSocket'] || jc.WebSocket) && !IS_ANDROID) {
        ...
        jc[ZERVER_KILL_STREAM] = function () {
            if (dying) {
                return;
            }
            dying = true;
    ...
    function setupAutoRefresh () {
        ...
        if (typeof jc[REFRESH_FUNC] !== 'function') {
            jc[REFRESH_FUNC] = function () {
                jc.location.reload();
            };
        }
    ...
    function setupLogging () {
        ...
        function interceptExceptions () {
            if ( !jc.addEventListener ) {
                return;
            }

            jc.addEventListener('error', function (msg, fileName, lineNum) {
                var evt;
                if (msg && msg.message) {
                    evt      = msg;
                    msg      = evt.message;
    ...
    function startReadyCheck () {
        if (!jc.addEventListener || (document.readyState === 'complete')) {
            flushReadyQueue();
        }
        else {
            jc.addEventListener('load', flushReadyQueue, false);
        }
    }

    function flushReadyQueue () {
        jc.removeEventListener('load', flushReadyQueue);
